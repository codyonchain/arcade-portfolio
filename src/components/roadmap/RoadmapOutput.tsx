...
export default function RoadmapOutput({
  ...
  const [nextPiecesOpen, setNextPiecesOpen] = useState(true);
  ...
  return (
    <div className="space-y-6">
      <div className="plastic glow-border p-6 space-y-5">
        ...
        {!snap ? (
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
              ...
              <div className="mt-4 text-xs text-white/55">Hooks turn ICP pain points into proof. Use them inside your hero copy + CTA.</div>
            </div>

            <CollapsiblePanel title="Next 8 plays" open={nextPiecesOpen} onToggle={() => setNextPiecesOpen((prev) => !prev)}>
              <div className="space-y-3">
                {snap.next8.map((topic) => (
                  <div key={topic.title} className="rounded-2xl border border-white/10 bg-black/10 p-4">
                    <div className="text-sm font-semibold text-white/90">{topic.title}</div>
                    <div className="mt-1 text-xs text-white/60">{topic.why}</div>
                    <div className="mt-3 text-xs text-white/55">
                      Score {topic.score.total}/15 • Pipeline {topic.score.pipelineFit}/5 • Authority {topic.score.authority}/5 • Ease {topic.score.ease}/5
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="plastic glow-border p-6 space-y-5">
        ...
        {!r30 ? (
          <div className="rounded-2xl border border-white/10 bg-black/20 p-5 text-sm text-white/70">Generate the 30-day roadmap to unlock schedule, briefs, and quick wins.</div>
        ) : (
          <>
            <CollapsiblePanel title="30-day schedule" open={scheduleOpen} onToggle={() => setScheduleOpen((prev) => !prev)}>
            ...
            <CollapsiblePanel title="Brief list" open={briefsOpen} onToggle={() => setBriefsOpen((prev) => !prev)}>
            ...
            <CollapsiblePanel title="Quick wins" open={quickWinsOpen} onToggle={() => setQuickWinsOpen((prev) => !prev)}>
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/20">
      <button
        type="button"
        onClick={onToggle}
        className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm font-semibold text-white/90"
      >
        <span>{title}</span>
        <span className="text-xs text-white/60">{open ? "−" : "+"}</span>
      </button>
      {open && <div className="border-t border-white/5 p-4 text-sm text-white/80">{children}</div>}
    </div>
  );
}
