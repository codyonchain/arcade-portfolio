    ...
    <div className="plastic glow-border p-6">
      ...
      <div className="mt-5 space-y-4">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <Text label="Ideal customer" value={inputs.icp} onChange={(v) => setField("icp", v)} placeholder="Who buys?" className="md:col-span-2" />
          <Text label="Differentiator" value={inputs.differentiator} onChange={(v) => setField("differentiator", v)} placeholder="Why you?" className="md:col-span-2" />
        </div>
        ...
        {error ? (
          ...
          <div className="text-xs text-white/45">Snap Plan = instant. Underwrite adds competitors, cadence, channels, briefs, packet.</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className={cn("rounded-2xl border border-white/10 bg-black/20 p-4", className)}>
      ...
      <input
        ...
        className="focus-ring mt-2 w-full rounded-xl border border-white/12 bg-black/20 px-3 py-2 text-sm text-white/90 placeholder:text-white/35"
      />
    </div>
  );
}
